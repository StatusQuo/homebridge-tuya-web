...
class CoverAccessory extends BaseAccessory {
  constructor(platform, homebridgeAccessory, deviceConfig) {
    ...
    this.service.getCharacteristic(Characteristic.CurrentPosition)
      .on('get', (callback) => {
        this.log.debug('[GET][%s] Characteristic.CurrentPosition: called', this.homebridgeAccessory.displayName);
        ...
        if (this.hasValidCache()) {
          ...
          this.platform.tuyaWebApi.getDeviceState(this.deviceId).then((data) => {
            ...
            */
            this.log.debug('[%s] STATE FROM API %s', this.homebridgeAccessory.displayName, data);
    ...
    this.service.getCharacteristic(Characteristic.PositionState)
      .on('get', (state, callback) => {
        ...
        this.log.debug('--- XXXXXX [GET][%s] Characteristic.PositionState: %s', this.homebridgeAccessory.displayName, data.state);
  ...
  updateState(data) {
    ...
    if (data.state) {
      ...
      const positionState = data.state - 1;
      ...
      // this.setCachedState(Characteristic.PositionState, positionState);
      // this.service
